...
final class GlobToRegex {
    ...
    private static final State BRACKET_FIRST_CHAR =
            new State() {
                ...
                void process(GlobToRegex converter, char c) {
                    if (c == ']') {
                        // A glob like "[]]" or "[]q]" is apparently fine in Unix (when used with ls
                        // for
                        // example) but doesn't work for the default java.nio.file implementations.
                        // In the cases
                        // of "[]]" it produces:
                        ...
                        // ^[[^/]&&[]]\]$
                        //              ^
                        // The error here is slightly different, but trying to make this work would
                        // require some
                        // kind of lookahead and break the simplicity of char-by-char conversion
                        // here. Also, if
                        // someone wants to include a ']' inside a character class, they should
                        // escape it.
