...
final class Directory extends File implements Iterable<DirectoryEntry> {
     ...
     * "..".
     */
    public SortedSet<Name> snapshot() {
        SortedSet<Name> builder = new TreeSet<>(Name.displayComparator());
    ...
    public Iterator<DirectoryEntry> iterator() {
        return new Iterator<DirectoryEntry>() {
            ...
            DirectoryEntry next = advance();

            private DirectoryEntry advance() {
                if (entry != null) {
                    entry = entry.next;
                }
