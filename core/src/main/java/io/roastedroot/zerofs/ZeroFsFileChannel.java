 ...
 * @author Colin Decker
 */
final class ZeroFsFileChannel extends FileChannel {
    ...
    public long read(ByteBuffer[] dsts, int offset, int length) throws IOException {
        if (offset < 0 || offset + length < offset || offset + length > dsts.length) {
    ...
    public long write(ByteBuffer[] srcs, int offset, int length) throws IOException {
        if (offset < 0 || (offset + length) < offset || (offset + length) > srcs.length) {
            throw new IndexOutOfBoundsException("Bad position indexes");
        }
    ...
    public MappedByteBuffer map(MapMode mode, long position, long size) throws IOException {
        // would like this to pretend to work, but can't create an implementation of
        // MappedByteBuffer
    ...
    static final class FakeFileLock extends FileLock {
        ...
        public FakeFileLock(
                AsynchronousFileChannel channel, long position, long size, boolean shared) {
            super(channel, position, size, shared);
        }
